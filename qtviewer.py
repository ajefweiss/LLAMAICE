...
from PyQt5.QtWidgets import QDialog, QApplication, QPushButton, QVBoxLayout, QWidget, QBoxLayout, QSpinBox, QHBoxLayout
...
def get_cme_by_id(id):
    myID = np.where(ICEdata[:,0].astype(int) == int(id))[0][0]
    myoIDs = np.where(otherData[:,0] == id)[0]
    ...
    return  id, myID, myoIDs, myYr, myDate, myDT

def gen_plot(fig, axes, CMEchoice, myID, myoIDs, myYr, myDate, myDT):
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, *args, **kwargs):
        ...
        self.button.clicked.connect(self.redraw)
        ...
        self.idbox = QSpinBox()
        self.idbox.setValue(int(cme_id))
        self.idbox.setRange(1, int(totalCMEs))
        self.idbox.valueChanged.connect(self.redraw)

        # sc.axes

        gen_plot(self.sc.fig, self.sc.axes, *choice)
    ...
    def redraw(self, new_id):
        gen_plot(self.sc.fig, self.sc.axes, *get_cme_by_id(str(new_id)))
...
if __name__ == '__main__':
    ...
    w = MainWindow(cme_id=sys.argv[1], choice=get_cme_by_id(sys.argv[1]))
    app.exec_()